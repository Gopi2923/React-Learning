...
## Episode 06
  ...
  - monolith architecture, microservices architectures
- it's known as separation of concerns
- loads => Render => API call => Render (for better ux experience we using this method in react)
- useEffect hook - it takes two arguments one is call back fn and dependencies. syntax: useEffect(() => {}, []);
- callback fn will call after the component renders.
- shimmer ui - loader components before loading
- whenever state variable changes its value, it will re-render the ui. That's why its a super powerful variable.
- whenever state variables update, react triggers a reconciliation cycle(re-render the component)
- search option, onchange
